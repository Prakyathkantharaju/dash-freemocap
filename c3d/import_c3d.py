...
class MarkerSet:
    ...
    def getMarker(self, marker, frame):
        idx = self.markerLabels.index(marker)
        return self.frames[frame - self.startFrame][idx].position

    def getMarkerList(self, marker_list, frame):

        idx = [self.markerLabels.index(c) for c in marker_list]
        return [self.frames[frame - self.startFrame][c].position for c in idx]
